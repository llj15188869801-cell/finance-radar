...
export default async function AdminPage() {
  ...
  return (
    <main className="page-shell admin-page">
      ...
      <section className="admin-panel"><div className="section-title"><h2>待审核热点</h2><button disabled>批量发布</button></div>{review.map((event: HotEvent) => <div className="admin-row" key={event.id}><span><b>{event.title}</b><small>置信度 {Math.round(event.confidence * 100)}% · {event.sourceCount} 个信源</small></span><div><button disabled>编辑</button><button disabled>发布</button></div></div>)}</section>
